...
export const api = {
  entreprises: {
    ...
    /** Étape 10 — active/désactive l'API publique pour une entreprise (superadmin). */
  ...
  apiKeys: {
    ...
    create: (data: { name: string; scopes: string[]; expires_at?: string | null; entreprise_id?: string }) =>
    ...
    rotate: (keyId: string, entreprise_id?: string) => {
      const qs = entreprise_id ? `?entreprise_id=${encodeURIComponent(entreprise_id)}` : "";
      return apiFetch<ApiKeyCreated>(`/api/api-keys/${keyId}/rotate${qs}`, { method: "POST" });
    },
...
export interface ApiUsageStats {
  ...
  calls_by_endpoint: { endpoint: string; count: number; pct: number; avg_response_ms: number | null }[];
  calls_by_key: { key_id: string; name: string; key_prefix: string; count: number; pct: number; last_call: string | null }[];
